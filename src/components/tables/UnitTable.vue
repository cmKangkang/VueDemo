...
import { getUnitList, editUnitInfos, addNewUnit, deleteUnit } 
  from '../../api/api.unit';
...
export default {
  ...
  methods:{
    ...
    deleteItem(item){
      this.$confirm('此操作将永久删除该单元文件, 是否继续?', '提示', {
          ...
          // 点击则移除该表项，并删除
          // 发送删除请求
          deleteUnit({"id":item.id}).then( data => {
            if( data.status==200 ){
              // 请求成功，移除本地表项
    ...
    enterNext(item){
      this.$router.push({name:'works',params:{'id':item.id }});
  ...
  created(){
    // 初始化items
    this.items.length=0;
    getUnitList({"id":this.id}).then( data => {
      console.log(data);
      let tmp=data.data;
      if( tmp.code=="1" ){
